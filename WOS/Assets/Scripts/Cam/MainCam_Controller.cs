...
public class MainCam_Controller : MonoBehaviour
{
    ...
    float myDist = 0.0f;
    public bool Follow_Possible = true;
    ...
    public void ChangeView(Transform target)
    {
        ...
        {
            Manager_Sound.Inst.BgmSource.OnPlay(5);
            Uis_OnOff(false);
        }
    ...
    IEnumerator Moving(Vector3 pos, bool ischangeview) // ischangeview = true -> ChangeView함수가 작동시킴 / ischangeview = false -> ReturnView함수가 작동시킴 
    {
        if (SaveTarget.TryGetComponent<Npc>(out Npc componet)) // Npc가 발동시킨거라면
        {
            if (ischangeview)
            {
                componet.Talk_Start();
