...
public class Item_Slot : MonoBehaviour, IDropHandler
{
    ...
    public void OnDrop(PointerEventData eventData)
    {
        ...
        if(NewItem.canDrag)
        {
            ...
            else
            {
                ...
                NewItem.transform.SetParent(NewItem.Before_Slot.transform);
    ...
    {
        if (item.GetComponent<Item2D_isQuantity>() != null)
        {
        ...
        Receive_Item(item.GetComponent<Item_2D>());
    }

    void Receive_Item(Item_2D item)
    {
        //sfx
        Manager_Sound.Inst.SfxSource.OnPlay(2);
