...
public class Player_Battle : Player_Movement, IBattle
{
    ...
    #region for Skill
    public override void OnSkill(int i)
    {
        //SkillRange, SkillPoint data 가져오기
        Skill_Set.Slots[i].OnSkill(SkillRange, SkillPoints, i);
        SkillPoints.OnSkill(i);
    ...
    IEnumerator Skilling()
    {
        ...
        {
            Ray ray = Camera.main.ScreenPointToRay(Input.mousePosition);
            if (Physics.Raycast(ray, out RaycastHit hit, 1000.0f, LayerMask.NameToLayer("SkillRange")))
            {
